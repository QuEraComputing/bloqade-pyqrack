...
class PyQrackMethods(interp.MethodTable):
    ...
    def swap(self, interp: interp.Interpreter, frame: interp.Frame, stmt: uop.Swap):
        qarg1: SimQubitRef["QrackSimulator"] = frame.get(stmt.qarg)
        qarg2: SimQubitRef["QrackSimulator"] = frame.get(stmt.ctrl)
    ...
    def cswap(self, interp: interp.Interpreter, frame: interp.Frame, stmt: uop.CSwap):
        qarg1: SimQubitRef["QrackSimulator"] = frame.get(stmt.qarg1)
        qarg2: SimQubitRef["QrackSimulator"] = frame.get(stmt.qarg2)
        ctrl: SimQubitRef["QrackSimulator"] = frame.get(stmt.ctrl)
        if qarg1.is_active() and qarg2.is_active():
    ...
    def rxx(self, interp: interp.Interpreter, frame: interp.Frame, stmt: uop.RXX):
        a: SimQubitRef["QrackSimulator"] = frame.get(stmt.qarg)
        b: SimQubitRef["QrackSimulator"] = frame.get(stmt.ctrl)
